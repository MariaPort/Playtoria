...
const updateGamesData = async (newLinksAdded) => {
  ...
  const extractGamesData = async (gameLink) => {
    ...
    const { description } = sourceData; // $('[data-test-id="app-description"]').text();
    const { name } = sourceData; // $('h1').text();
    const publisherName = sourceData.author.name;
    // eslint-disable-next-line max-len
    // $('body > div:nth-child(1) > main > div.space-y-6.mt-6 > astro-island > div > div > div:nth-child(1) > span > div > div > a').text();
    const categoryName = $('div[data-cy="app-profile-info-app-category"]:nth-child(2) > span > div > a').text();
    const { storeType } = gameLink;
